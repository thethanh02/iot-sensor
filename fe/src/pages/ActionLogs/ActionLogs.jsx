...
const ActionLogs = () => {
  ...
  const handleNextPage = async () => {
    if (date && currentPage < totalPages) {
      await Api.getActions(date, currentPage + 1, dateSorted)
        .then(res => {
          setData(res.data)
  ...
  const handleLastPage = async () => {
    if (date && currentPage < totalPages) {
      await Api.getActions(date, totalPages, dateSorted)
        .then(res => {
          setData(res.data)
  ...
  const handleFirstPage = async () => {
    if (date && currentPage > 0) {
      await Api.getActions(date, 0, dateSorted)
        .then(res => {
          setData(res.data)
          setCurrentPage(0)
        }).catch(error => console.log(error))
  ...
  React.useEffect(() => {
    Api.getActions(date, currentPage, dateSorted)
  ...
  const handleClickSearch = async () => {
    ...
    if (date && date.from && date.to) {
      ...
      await Api.getTotalPagesActions(date)
        .then(res => {
  ...
  const handleSortByDate = async () => {
    await Api.getActions(date, 0, 1 - dateSorted)
      .then(res => {
        setData(res.data)
  ...
  return (
    <div className="h-full w-full bg-gray-50 px-3 py-5 xl:px-16 xl:py-12">
      <header className="ie-as-header flex w-full justify-between">
        <h3 className="text-xl font-semibold text-gray-900">All Action Logs</h3>
      </header>
